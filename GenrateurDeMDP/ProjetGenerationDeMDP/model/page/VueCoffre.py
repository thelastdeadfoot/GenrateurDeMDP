...
class VueCoffre:
    ...
    def afficher_menu(self, event):
        ...
        if col_index == 0  or col_index == 1 or col_index == 2 :
            ...
            self.menu.add_command(label="Supprimer la ligne", command=lambda: self.supprimer_thread(row))
        else :
            self.menu.add_command(label="Supprimer la ligne", command=lambda: self.supprimer_thread(row))
    ...
    def modifier_cellule(self, row, col):
        ...
        entry.bind("<Return>", lambda e: self.sauvegarder_thread(row, col_index, col_nom, entry))
        # Détruire l'Entry en cas de perte de focus
        entry.bind("<FocusOut>", lambda e: entry.destroy())

    def sauvegarder_thread(self, row, col_index, col_nom,entry):
        thread = threading.Thread(target=self.sauvegarder_edit, args=(row, col_index, col_nom,entry))
        thread.start()

    def supprimer_thread(self, row):
        thread = threading.Thread(target=self.supprimer_ligne, args=(row,))
        thread.start()

    def sauvegarder_edit(self, row, col_index, col_nom,entry):
        new_text = entry.get()
