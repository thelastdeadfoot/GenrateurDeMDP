...
class SiteDao:
    ...
    # Recupere le nom du site avec son id
    def recup_nom_site(self, site_nom_a_chercher):
        response = supabase.table("Site").select("nomSite").eq("idSite", site_nom_a_chercher).execute()
        if response.data and len(response.data) > 0:
            e = response.data[0]["nomSite"]
            return e

    # Met a jour le nom d'un site
