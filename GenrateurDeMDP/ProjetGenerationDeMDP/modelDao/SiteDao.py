...
class SiteDao:
    ...
    def verifSite(self, existeSite):
        print("verif si existe : " + existeSite)
        response = supabase.table("Site").select("nomSite").eq("nomSite", existeSite).execute()
        print(response.data)
    ...
    def recupIdSite(self, siteIdAchercher):
        print("Récupération des données de " + "\'"+siteIdAchercher+"\'")
        response = supabase.table("Site").select("idSite").eq("nomSite", siteIdAchercher).execute()
        if response.data and len(response.data) > 0:
