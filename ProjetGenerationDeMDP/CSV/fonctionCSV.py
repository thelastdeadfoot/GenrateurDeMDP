...
from modelDao.SiteDao import SiteDao

#faut voir avec la prof si on en fait une classe
...
def extraction(NomFichier):
    ...
    #la partie que je prendrais du login de l'utilisateur qui seras rentrée sur l'application
    #df[idUtilisateur] = utilisateurDao.recupIdUtilisateur(utilisateur.getLogin)
    print(df.to_string(max_rows=20))
    return df

def envoieMdpViaCSV(nomFichierCSV):
    # du coup voici comment ça marche
    # fait tout les modif
    fichierTraite = extraction(nomFichierCSV)
    ...
    load_data(fichier, "MotDePasse",'"mdp", "idSite", "categorie", "nbCaractere", "nbNum", "nbCarSpe", "idUtilisateur", "Robustesse", "carMini", "carMaj"')

#envoieMdpViaCSV('mdp$.csv')